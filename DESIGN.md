...
固定侧栏（204px），顶栏（65px），内容最大宽度（1680px），桌面内容内边距（30px 32px）。主要编辑网格为弹性主列与试跑列（390px），列间距（24px）；当前主列 CSS 最小值为（450px）。面板内边距（24px），字段标签间距（8px）。8px 是基础节奏，控件内部保留现有光学调整。

桌面宽度达到（1280px）时，试跑区在右侧并以距顶（22px）吸附。低于此宽度，试跑区移至编辑器下方；（601–1279px）范围内将**完整输入组**与**完整结果组**分成两列，组内顺序保持不变，不将标题、编辑器、按钮、结果标签拆成交错网格。

不超过（900px）时，侧栏收为图标栏（68px），内容边距收至（24px）。不超过（600px）时，输入与结果上下排列，恢复结果上方分隔线；内容边距（18px 14px），面板内边距（18px）。运行端选择从四列变为两列，再变为单列。表格和标签条允许区域内横向滚动。

授权行在宽屏排列勾选项、函数 key、版本选择；手机上三者逐项堆叠。勾选文本不挤成逐字竖排，长 key 可换行，版本选择占满可用宽度。手机范围保证阅读，完整编辑验收下限仍为（768px）。
